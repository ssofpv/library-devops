...
- Книга и филиал могут иметь от нуля до нескольких экземпляров.
...
Аутентификация использует стандартные таблицы Django `auth_user` и `django_session`.
Логин уникален, пароль хранится в виде хеша; `is_active` разрешает вход.
Сессионные данные и срок действия хранятся на сервере, браузер получает cookie sessionid.
Собственная модель пользователя и новые миграции не нужны: таблицы создаёт `manage.py migrate`.
Сотрудники работают с общим каталогом; связи «владелец книги» и разделения по пользователям нет.
